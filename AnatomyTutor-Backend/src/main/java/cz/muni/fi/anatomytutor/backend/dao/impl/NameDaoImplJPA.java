...
import cz.muni.fi.anatomytutor.backend.dao.NameDao;
import cz.muni.fi.anatomytutor.backend.model.Name;
 ...
 * @author Jan Kucera
 */
public class NameDaoImplJPA implements NameDao {

    final static Logger log = LoggerFactory.getLogger(NameDaoImplJPA.class);
    // injected from Spring
    @PersistenceContext
    private EntityManager em;

    public NameDaoImplJPA() {
    }

    public NameDaoImplJPA(EntityManager em) {
        this.em = em;
    }

    public Long create(Name name) {
        if (name == null) {
            throw new IllegalArgumentException("Invalid name: null");
        }
        Name createdName = em.merge(name);
    ...
    public Name get(Long id) {
        ...
        return em.createQuery("SELECT tbl FROM Name tbl "
                + "WHERE tbl.id = :givenId", Name.class).setParameter("givenId", id).getSingleResult();
    }
